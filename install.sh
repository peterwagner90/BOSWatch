...
apt-get -y install git cmake build-essential libusb-1.0-0-dev qt5-qmake qt5-default libpulse-dev libx11-dev sox python3-pip >> $boswatch_install_path/setup_log.txt 2>&1
...
case $branch in
  "dev") git clone -b develop https://github.com/peterwagner90/BOSWatch.git . >> $boswatch_install_path/setup_log.txt 2>&1 && \
    exitcodefunction $? git-clone BOSWatch-develop ;;
  *) git clone -b master https://github.com/peterwagner90/BOSWatch.git . >> $boswatch_install_path/setup_log.txt 2>&1 && \
