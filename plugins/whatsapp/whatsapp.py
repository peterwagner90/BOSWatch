...
"""
plugin to send messages to a whatsapp group
...
def onLoad():
	...
	try:
		########## User onLoad CODE ##########
		if configHandler.checkConfig("whatsapp"):
			logging.debug(globalVars.config.get("whatsapp", "group"))
			pass
		########## User onLoad CODE ##########
	except:
	...
	try:
		if configHandler.checkConfig("fcm"): #read and debug the config (let empty if no config used)
			...
			if typ == "FMS":
				...
				message = data["msg"]
				response = pywhatkit.sendwhatmsg_to_group(globalVars.config.get("whatsapp", "group"), message)	
				logging.info("WhatsApp Message sent %s", response)
			else:
